...
shared_ptr<Figure> CreateFigure(istringstream& input)
{
	...
	input >> name_of_figure;
	if(name_of_figure == "RECT"){
		double width, height;
		input >> width >> height;
		res = make_shared<Rect>("RECT", width, height);
	}
	else if(name_of_figure == "TRIANGLE"){
		double a, b, c;
		input >> a >> b >> c;
		res = make_shared<Triangle>("TRIANGLE", a, b, c);
	}
	else{
		double r;
		input >> r;
		res = make_shared<Circle>("CIRCLE", r);
	}
	return res;
}
