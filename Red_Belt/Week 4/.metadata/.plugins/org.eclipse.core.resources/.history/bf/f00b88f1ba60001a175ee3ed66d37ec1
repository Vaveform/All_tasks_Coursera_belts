...
public:
  ...
  const size_t& Get(TAirport airport) const{
	  return Stat[uint32_t(airport)].second;
  }

  void Insert(TAirport airport){
	  Stat[uint32_t(airport)].second += 1;
  }

  void EraseOne(TAirport airport){
	  if(Stat[uint32_t(airport)].second != 0){
		  Stat[uint32_t(airport)].second -= 1;
	  }
  }

  void EraseAll(TAirport airport){
	  Stat[uint32_t(airport)].second = 0;
  }

  using Item = pair<TAirport, size_t>;
  using Items = array<Item, uint32_t(TAirport::Last_)>;
