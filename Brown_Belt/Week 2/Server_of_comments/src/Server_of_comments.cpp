...
class HttpResponse {
public:
  explicit HttpResponse(HttpCode code) : code_(move(code)), content_("") {};
  ...
  unordered_multimap<string,string>& GetHeaders();
  string& GetContent();

  friend ostream& operator << (ostream& output,const HttpResponse& resp);
private:
  string protocol_ = "HTTP/1.1";
  HttpCode code_;
...
HttpResponse& HttpResponse::AddHeader(string name, string value){
	headers_.emplace(make_pair(move(name), move(value)));
	return *this;
}
...
ostream& operator << (ostream& output,const HttpResponse& resp){
	if(resp.code_ == HttpCode::Found){
		output << resp.protocol_ << " " << (int)resp.code_ << " Found" << "\n";
	}
	else if(resp.code_ == HttpCode::NotFound){
		output << resp.protocol_ << " " << (int)resp.code_ << " Not found" << "\n";
	}
	else{
		output << resp.protocol_ << " " << (int)resp.code_ << " OK" << "\n";
	}
	size_t s = 0;
	for(auto it = resp.headers_.begin(); it != resp.headers_.end(); it++){
		if(s == resp.headers_.size() - 1){
	...
	if(resp.content_.empty() && resp.headers_.empty()){
		output << "\n";
	}
...
unordered_multimap<string,string>& HttpResponse::GetHeaders(){
...
////  stringstream ss;
//  HttpResponse hr = srv.ServeRequest(request);
////  ParsedResponse resp;
////  ss >> resp;
//  vector<HttpHeader> headers;
//  for(const auto& element : hr.GetHeaders()){
//	  headers.emplace_back(HttpHeader{element.first, element.second});
//  }
//  ASSERT_EQUAL((int)hr.GetHttpCode(), expected.code);
//  ASSERT_EQUAL(headers, expected.headers);
//  ASSERT_EQUAL(hr.GetContent(), expected.content);
//  cout << hr << endl;
//}
//
