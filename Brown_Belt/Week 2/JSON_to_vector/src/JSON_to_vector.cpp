...
vector<Spending> LoadFromJson(istream& input) {
	Document json_doc = Load(input);
	vector<Spending> spends;
	for(const auto& element : json_doc.GetRoot().AsArray()){
		const map<string, Node>& pair_node = element.AsMap();
		spends.push_back({pair_node.rbegin()->second.AsString(), pair_node.begin()->second.AsInt()});
	}
	return spends;
}
