...
public:
  ...
  struct WriteAccess {
	lock_guard<mutex> g_m_a;
    V& ref_to_value;
  };

  struct ReadAccess {
	lock_guard<mutex> g_m_r;
    const V& ref_to_value;
  };

  explicit ConcurrentMap(size_t bucket_count) : hash_tables(bucket_count), mutexes(bucket_count){};

  WriteAccess operator[](const K& key){
	  return {lock_guard<mutex>(mutexes[hasher(key) % hash_tables.size()]), hash_tables[hasher(key) % hash_tables.size()][key]};
  };
  ReadAccess At(const K& key) const{
	  return {lock_guard<mutex>(mutexes[hasher(key) % hash_tables.size()]),hash_tables[hasher(key) % hash_tables.size()].at(key)};
  }

  bool Has(const K& key) const{
	  return hash_tables[hasher(key) % hash_tables.size()].count(key) == 0 ? false : true;
  }
