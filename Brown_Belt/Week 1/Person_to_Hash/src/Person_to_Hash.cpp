...
struct AddressHasher {
	size_t operator()(const Address& addr) const {
		int simple_num = 50847534;
		return hash<string>{}(addr.city)*simple_num*simple_num +
				hash<string>{}(addr.street)*simple_num + hash<int>{}(addr.building);
	}
};
...
struct PersonHasher {
  size_t operator()(const Person& p) const{
	  int x = 50847534;
	  return hash<string>{}(p.name)*x*x*x +
			  hash<int>{}(p.height)*x*x +
			  hash<double>{}(p.weight)*x +
