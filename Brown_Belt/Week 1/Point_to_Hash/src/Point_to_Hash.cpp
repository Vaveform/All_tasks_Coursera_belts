...
struct Hasher {
  size_t operator()(const Point3D& p) const{
	  // Простое число для полинома C + B*x + A*x^2, где x - и есть простое число
	  // Нужно стараться брать максимально большое число
	  int x = 50847534;
	  return hash<CoordType>{}(p.x)*x*x + hash<CoordType>{}(p.y)*x + hash<CoordType>{}(p.z);
  }
};
