...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_spends;
...
void Push(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      ...
      tree_spends[w] += tree_spends[v];
      (tree_values[w] *= tree_factor[v]) += tree_add[v] * (r - l) / 2;
      tree_values[w] -= tree_spends[v] * (r - l) / 2;
    }
  }
  tree_factor[v] = 1;
  tree_add[v] = 0;
...
void Substract(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  ...
  Substract(v * 2, l, (l + r) / 2, ql, qr, value);
  Substract(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : 0)
      + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : 0);
}
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double tax_percentage) {
  ...
  if (ql <= l && r <= qr) {
    ...
    tree_values[v] *= (1.0 - tax_percentage / 100.0);
    return;
  }
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      ...
      double tax_percent;
      cin >> tax_percent;
      Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, tax_percent);
    }
      ...
      double value;
      cin >> value;
      Substract(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
    }
  }
