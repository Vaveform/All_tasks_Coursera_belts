...
struct BulkTaxApplier {
  ...
  double ComputeFactor() const {
    return (1.0 - tax_percentage / 100.0);
  }
};
...
public:
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    tax_.tax_percentage = 100.0 - tax_.ComputeFactor() * other.tax_.ComputeFactor() * 100;
    add_.delta.earned = add_.delta.earned * other.tax_.ComputeFactor() + other.add_.delta.earned;
    add_.delta.spent += other.add_.delta.spent;
  }

  MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
    return {origin.earned * tax_.ComputeFactor() + add_.delta.earned * segment.length(),
...
struct EarnRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    //cout << "Length: " << date_segment.length() << endl;
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{tax_percentage});
  }
...
struct SpendRequest : ModifyRequest{
	...
	void Process(BudgetManager& manager) const override{
		const auto date_segment = MakeDateSegment(date_from, date_to);
		const double daily_spend = 1.0 * spent / date_segment.length();
